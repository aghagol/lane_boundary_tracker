...
# ------------------------------------------------------------------------------
def dynamic_import(module_path, package_path=None):
    """
    Import module or function based on string

    Args:
        module_path (str): Module to import

    Returns:
        module: Module or function that was imported

    """
    path, module_name = module_path.rsplit('.', 1)

    module = import_module(path, package_path)
    return getattr(module, module_name)


# ------------------------------------------------------------------------------
def get_params_from_command_line(package):
    """
    Get the parameters module from the command line options

    Args:
        package (str): Package used for relative import of params

    Returns:
        Params: Params object from module specified by the command line
    ...
    return dynamic_import('.params_dir.{}.params'.format(args.params), package)
