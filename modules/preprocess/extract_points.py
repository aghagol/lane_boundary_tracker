...
def run(images, fuses, config, drives, output, verbosity):
    if not os.path.exists(fuses):
        os.makedirs(fuses)
    ...
    image_to_fuse_map = os.path.join(output, 'img2fuse.txt')
    ...
    for drive in drive_list:
        ...
        for n, res in meta.iterrows():
            ...
            if not os.path.exists(os.path.join(images, drive, res['name'])): continue
            ...
            if os.path.exists(os.path.join(fuses, fuse_filename)): continue

            if verbosity >= 2:
                print('\tworking on %s' % (res['name']))

            # read the probability image (FCN output)
            pred_im = misc.imread(os.path.join(images, drive, res['name'])) / (2. ** 16 - 1)
            ...
            np.savetxt(os.path.join(fuses, fuse_filename), lat_lon, fmt=fuse_fmt, delimiter=',')
...
def main(argv):
    ...
    run(args.images, args.fuses, args.config, args.drives, args.output, args.verbosity)
